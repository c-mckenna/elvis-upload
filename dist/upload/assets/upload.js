...

{
  ...
  .config(['projectsServiceProvider', 'versionServiceProvider', 'persistServiceProvider', function (projectsServiceProvider, versionServiceProvider, persistServiceProvider) {
    ...
    try {
      var delay = userService.expires().getTime() - Date.now(); // console.log("Delay = " + delay)
  ...
  var SubmitService = /*#__PURE__*/function () {
    ...
    _createClass(SubmitService, [{
      ...
      value: function upload(file, token) {
        ...
        postData.append("file", file); // ADD LISTENERS.

        var objXhr = new XMLHttpRequest(); //objXhr.addEventListener("progress", updateProgress, false);

        objXhr.addEventListener("load", transferComplete, false); // SEND FILE DETAILS TO THE API.

        objXhr.open("POST", config.uploadUrl + "?token=" + token);
        objXhr.send(postData);
        var promise = this.$q.defer();
        return promise.promise; // CONFIRMATION.
